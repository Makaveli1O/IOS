...
#--------TO DO ----------------#
# PRE súbory veľkosti 0 !!!
# UPRAVIT VYPIS PODLA ZADANIA !!!!
# zavolanie na "-n ~/Documents/VUTBR/2.year" vynechá jednu mriežku nvm prečo
...
#--DEBUUUUG--#
echo "i_filter = "$i_filter
echo "i_val = "$i_val
echo "n_filter = "$n_filter
echo "DIR = "$DIR
#----DEBUF---
...
function Rounding(){
	...
	#TERMINAL is not being used
		...
		#if normalization is still not enough recursively call Rounding again
		[[ $num_hashes -gt 79 ]] && Rounding $1 $num_hashes
...
empty_directories=$(find $DIR -empty | wc -l)	#I probably won't need this
...
then
	...
	then
		...
		file_size=$(ls -lR | sed '/\.\/'$i_val'/, /^$/'d | egrep -v '^d' | awk '$5>0{print $5}')
	else
		file_size=$(ls -lR | egrep -v '^d' | awk '$5>0{print $5}')
	fi
else
	if $i_filter
	then
		...
		file_size=$(ls -lR $DIR | sed "/$DIR2\/$i_val/,/^$/"d | egrep -v '^d' | awk '$5>0{print $5}')
	else
		file_size=$(ls -lR $DIR | egrep -v '^d' | awk '$5>0{print $5}')
	fi
fi
